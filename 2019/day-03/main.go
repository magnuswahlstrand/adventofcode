...
func getSteps(s string) (coord, int) {
	d := dir[s[0]]
...
func findClosest(l1, l2 string) (int, int) {
	...
	pos := startingPos
	tot := 1
	for _, s := range strings.Split(l1, ",") {
		d, steps := getSteps(s)
...
func main() {
	...
	closest, signalClosest = findClosest(l1, l2)
	log.Println("Solution part 1:", closest)
	log.Println("Solution part 2:", signalClosest)
}
