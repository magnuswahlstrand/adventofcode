...
func parse(in string) *node {
	...
	var childrenMap = make(map[string][]string)

	for _, row := range strings.Split(in, "\n") {

		fields := strings.Fields(strings.Replace(row, ",", "", -1))
		name := fields[0]
		value, err := strconv.Atoi(fields[1][1 : len(fields[1])-1])
		if err != nil {
			log.Fatal(err)
		}
...
func main() {
	...
	// fmt.Println("Found top node azqje") // Took 20 minutes
}
...
func (n *node) weight() int {
	...
	for i, w := range ws {
		next := ws[(i+1)%len(ws)]
		nextnext := ws[(i+2)%len(ws)]
		if next == nextnext && w != next {
			fmt.Printf("Node %s is not balanced. It weighs %d. It is differs by %d. It should be %d\n",
				n.children[i].name, w, w-next, n.children[i].value-(w-next))
		}
	}
