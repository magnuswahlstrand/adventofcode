...
func validate(in string) bool {
	...
	for _, s := range strings.Split(in, " ") {
		sorted := orderedWord(s)
		if words[s] > 0 || words[sorted] > 0 {
			return false
		}
		words[s]++
		words[sorted]++
...
func orderedWord(s string) string {
	var rs []rune
	for _, r := range s {
		rs = append(rs, r)
	}
...
abcde fghij
abcde xyz ecdab`
	fmt.Println("Valid passphrases:", validateMultiple(ins))

	fmt.Println(orderedWord("abcd"), orderedWord("dbca"))
	fmt.Println("Valid passphrases:", validateMultiple(input))
	// Part 1 - 7 min
	// Part 2 - 15 min

}
func validateMultiple(ins string) int {

	var c int
	for _, row := range strings.Split(ins, "\n") {
		if validate(row) {
			c++
		}
	}
...
var input = `vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
